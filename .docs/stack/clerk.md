...
- **Create** a `middleware.ts` file using `clerkMiddleware()` from `@clerk/nextjs/server`. **CRITICAL**: If the project uses `src/` directory, the middleware MUST be at `src/middleware.ts` — Clerk will NOT find it at root `./middleware.ts` and will throw: *"clerkMiddleware() was not run, your middleware file might be misplaced"*.
...
Create your `middleware.ts` file (use `src/middleware.ts` when project has `src/` directory):

```typescript
// src/middleware.ts (or middleware.ts at root if no src/)
...
1. **Use `clerkMiddleware()`** from `@clerk/nextjs/server` in `middleware.ts`. When using `src/`, place file at `src/middleware.ts` — root `./middleware.ts` causes Clerk to fail.
...
## **3. LẤY ROLE / PHÂN QUYỀN (Common Bug)**

**Vấn đề:** `sessionClaims?.metadata?.role` trả về `undefined` dù đã set `publicMetadata.role` trong Clerk Dashboard.

**Nguyên nhân:** `sessionClaims.metadata` chỉ có khi đã cấu hình **Sessions → Customize session token** với claim `{"metadata": "{{user.public_metadata}}"}`. Nếu chưa cấu hình thì `metadata` không tồn tại.

**Fix:** Dùng `currentUser().publicMetadata.role` thay vì `sessionClaims.metadata.role`. Không cần cấu hình session token.

```typescript
// ✅ Đúng - hoạt động ngay, chỉ cần set publicMetadata cho user
const user = await currentUser();
const role = (user?.publicMetadata?.role as string) ?? "user";

// ❌ Sai khi chưa customize session token
const { sessionClaims } = await auth();
const role = sessionClaims?.metadata?.role; // undefined
```

---

## **4. MIDDLEWARE FILE LOCATION (Common Bug)**

**Error:** `Clerk: clerkMiddleware() was not run, your middleware file might be misplaced. Move your middleware file to ./src/middleware.ts. Currently located at ./middleware.ts`

**Cause:** Next.js projects with `src/` directory require middleware at `src/middleware.ts`. Root `./middleware.ts` is not picked up by Clerk.

**Fix:** Move `middleware.ts` from project root to `src/middleware.ts`.

| Project structure | Middleware path     |
|-------------------|---------------------|
| Has `src/` dir    | `src/middleware.ts` |
| No `src/` dir     | `middleware.ts` (root) |
...
1. **Middleware**: Is `clerkMiddleware()` used in `middleware.ts`? If project has `src/`, is file at `src/middleware.ts`?
